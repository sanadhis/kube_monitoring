...
def filter_request(func):
    """
    Decorator method for main function.
    Filter non-POST request.
    Args:
        func  (function)  : method main
    """

    def check(request):
        """
        Function to Filter non HTTP POST request.
        Args:
        ...
        # If POST, proceed the function without modifying the arguments.
        else:     
            return func(request)
...
def verify_credentials(func):
    """
    Decorator method for main function.
    Check and match request credentials with one that provided in environment
    Args:
        func  (function)  : method main
    """
    ...
    def check(request):
        """
        Check credentials of a request.
        Args:
            request  (HttpRequest)  : User http request.
        Returns:
            response (JsonResponse) : Response for the request in JSON format.
        ...
        # case if credentials matched with setting in environment
        else:
            # proceed the function without modifying the arguments.
            return func(request)

    return check

# Main function
@method_decorator(csrf_exempt, name='dispatch')    # Allow cross-site request forgery
@filter_request                                    # Filter request, reject non-POST request
@verify_credentials                                # Verify request username and password in request header
def main(request):
    """
